...
describe('Browser Navigation Comprehensive Tests', () => {
  ...
  describe('Basic Navigation', () => {
    ...
    test('should handle navigation to non-existent domain', async () => {
      const result = await client.callTool({
        name: 'browser_navigate',
        arguments: {
  ...
  describe('Navigation History', () => {
    test('should navigate forward and backward through history', async () => {
      ...
      expect((content.content as any)[0].text).toContain('Welcome to Home');
  ...
  describe('Page Content Retrieval', () => {
    ...
    test('should get HTML content of page', async () => {
      ...
      expect(result.isError).toBe(false);
      expect((result.content as any)[0].text).toContain('<html>');
      expect((result.content as any)[0].text).toContain('Welcome to Home');
    });
    ...
    test('should get text content of page', async () => {
      ...
      expect(result.isError).toBe(false);
      expect((result.content as any)[0].text).toContain('Welcome to Home');
      expect((result.content as any)[0].text).not.toContain('<html>');
    });
